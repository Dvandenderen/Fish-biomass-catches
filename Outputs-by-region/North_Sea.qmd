...
Time series of marine fish catches and landings were estimated using data from SAU and Watson (2017). The catch estimates encompass landings, discards, and approximations of illegal, unregulated, and unreported (IUU) catches.
...
Bio_STO$area <- sum(LMEgrid$ocean_sqkm[LMEgrid$depth >= -500])
...
#rm few hauls from FR-CGFS and SWC-IBTS
Reg <- subset(Reg,Reg$Survey_Region %in% c("BITS","NorBTS","NS-IBTS"))
...
if (!file.exists(paste0("../Models_sdmtmb/stocks_",LME_name,".RData"))) {
  ...
  model_fit <- sdmTMB(
    stocks_kg_km2 ~ 0 + Yearf + Surveyf + (1|Monthf) + 
...
Time series of total fish biomass were estimated by LME using data from scientific trawl surveys in a similar way as above but now for all fish in the survey.
...
Time series of the fish community exploitation rate were estimated by LME by dividing catches over total biomass. This was done using catch data from SAU and Watson, survey data and stock assessment information.
...
In the catch statistics, demersal fish account for approximately 60% of the total catch, while in the survey data, they represent around 70%. This is a relatively minor discrepancy, which is likely due to the underestimation of pelagic fish in survey data.
