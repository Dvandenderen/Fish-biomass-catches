---
title: "Gulf of Maine and Bay of Fundy biomass and catches"
format: html
...
# define values
ME_name   <- "Gulf of Maine_Bay of Fundy"
nam       <- "Gulf of Maine/Bay of Fundy" # include two names to avoid issues with dash
Start_year <- 1984
End_year <- 2015
...
# -----------------------------------------------------
# load SAU data
SAU <- read.csv("../Data/SAU/SAU_GulfMaine_BFundy_ME.csv")
...
# -----------------------------------------------------
# load Watson
load("../Data/Watson/GulfMaineBayFundyME.RData")
...
STO <- subset(grid,grid$ME %in% nam)
...
totdat <- cbind(totdat,conv[match(totdat$Stock,conv$stocklong), c("conversion_final")]) 
colnames(totdat)[ncol(totdat)] <- "conversion"
totdat$Tbio <- totdat$Tbio/totdat$conversion

# now combine
MEgrid <- subset(grid,grid$ME == nam)
...
grid <- get_grid_ME(ME_name = nam , grid_size_km = 10)
...
grid$Surveyf <- as.factor("Northeast US Spring")
...
lines(Catch_STO$Tonnes/Bio_STO$Tbio~Bio_STO$Year,lty=1,lwd=2,col="black")
...
# get it for the survey - shortcut
  agg <- aggregate(
    x = list(dem = Reg$dem+Reg$dem_stock, pel = Reg$pel+Reg$pel_stock),
