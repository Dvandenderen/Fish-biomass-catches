---
title: "East Bering Sea biomass and catches"
format: html
...
The catch figure includes a time series of catch for all assessed stocks for which total biomass or spawning stock biomass estimates are available. Some of these stocks have spatial assessment areas that only partially overlap with the area. In such cases, catch were proportionally adjusted based on the degree of overlap. For each stock, catch were allocated only to areas where the species occur. For example, catch from stocks inhabiting shallow waters were not assigned to deeper areas. This allocation was based on distribution maps from AquaMaps.
...
# subset Reg to only get EBS
Reg <- subset(Reg,Reg$Haul_id %in% c(hauls$Haul_id[ha_grid[,1]]))
Reg <- subset(Reg,!(Reg$Longitude < -175 & Reg$Latitude < 54))
...
Reg <- Reg %>%
  pivot_wider(names_from = F_type, 
    ...
    mutate(Total_kg_km2 = rowSums(dplyr::select(., c("dem","pel", #"pel_stock",
...
In all East Bering Sea statistics, demersal fish account for \>90% of the total catch.
...
# get it for the survey - shortcut
  agg <- aggregate(
    x = list(dem = Reg$dem+Reg$dem_stock, pel = Reg$pel),
