---
title: "Celtic-Biscay shelf biomass and catches"
format: html
...
# -----------------------------------------------------
# load SAU data
SAU <- read.csv("../Data/SAU/SAU_CelticBiscay.csv")
...
Time series of assessed fish biomass were estimated by LME using data from scientific trawl surveys and based on information for all assessed stocks for which total biomass or spawning stock biomass estimates are available in the Celtic-Biscay Shelf. Some of these stocks have spatial assessment areas that only partially overlap with the Celtic-Biscay Shelf. In such cases, biomass were proportionally adjusted based on the degree of overlap. For each stock, biomass were allocated only to areas where the species occur. For example, biomass from stocks inhabiting shallow waters were not assigned to deeper areas. This allocation was based on distribution maps from AquaMaps.
...
if (!file.exists(paste0("../Models_sdmtmb/stocks_",LME_name,".RData"))) {
  ...
  model_fit <- sdmTMB(
    ...
    data = Reg,                     
    mesh = spatial_mesh,                
    family = tweedie(link = "log"),           # to deal with zero's
...
In both the catch statistics and the survey data, demersal fish account for approximately 60% of the total catch/biomass.
