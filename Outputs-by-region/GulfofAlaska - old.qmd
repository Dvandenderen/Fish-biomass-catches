---
title: "Gulf of Alaska biomass and catches"
format: html
...
# -----------------------------------------------------
# load SAU data
SAU <- read.csv("../Data/SAU/SAU_GulfAlaska.csv")
...
# -----------------------------------------------------
# load Watson
load("../Data/Watson/GulfofAlaskaME.RData")
...
stockid <- unique(as.character(stock[1,][grepl("Gulf of Alaska",stock[1,])]))
...
tr <- subset(tr,tr$unitF %in% c("Metric Tons","Thousand Metric Tons"))
tr <- subset(tr,tr$unitA %in% c("Metric Tons","Thousand Metric Tons"))
tr$Fmort  <- ifelse(tr$unitF == "Thousand Metric Tons",tr$Fmort*1000, tr$Fmort)
tr$unitF  <- ifelse(tr$unitF == "Thousand Metric Tons","Metric Tons", tr$unitF)
tr$Abundance  <- ifelse(tr$unitA == "Thousand Metric Tons",tr$Abundance*1000, tr$Abundance)
tr$unitA  <- ifelse(tr$unitA == "Thousand Metric Tons","Metric Tons", tr$unitA)

# and walleye pollock in Eastern Bering Sea has a unit issue
tr$Fmort <- ifelse(tr$stocklong == "Walleye pollock - Eastern Bering Sea",tr$Fmort*1000,tr$Fmort)
...
dem_stock <- c("Atheresthes stomias","Beringraja binoculata",
               "Microstomus pacificus","Sebastes ciliatus",
               "Hippoglossoides elassodon","Raja rhina",
               "Lepidopsetta polyxystra","Sebastes polyspinis",
               "Gadus macrocephalus", "Sebastes alutus",
               "Glyptocephalus zachirus","Lepidopsetta bilineata",
               "Sebastes borealis","Sebastolobus alascanus",
               "Gadus chalcogrammus","Sebastes ruberrimus")
...
grid$col <- ifelse(grid$Depth >= -200,"blue","red")
...
plot(tr$Bio/stck_index$area[1]~tr$Year,ylim=c(0,80),type="l",col="black",
