...
STO <- subset(grid,grid$ME %in% c(ME_name1,ME_name2))
...
stockid <- as.character(stock[1,][grepl("Bering|Aleutian", stock[1,])])
...
stockdat <- subset(stockdat,stockdat$Year %in% c(Start_year:End_year))

# correct and combine
tr <- subset(stockdat,!(is.na(stockdat$Fmort)))
tr <- subset(tr,!(is.na(tr$Abundance)))
tr$Abundance <- ifelse(tr$unitA_des == "Mature Female Biomass",tr$Abundance *2,tr$Abundance )
tr <- subset(tr,tr$unitF == "Metric Tons")
tr <- subset(tr,tr$unitA == "Metric Tons")
tr <- aggregate(list(tr$Fmort,tr$Abundance),by=list(tr$Year),FUN=sum)      
colnames(tr) <- c("Year","Catch","Bio")
tr$ER <- tr$Catch/tr$Bio
Catch_STO <- data.frame(Tonnes = tr$Catch, Year = tr$Year)
...
load("../Data/NOAA_stocks/AquaMaps_occurence_NOAA_stocks.Rdata")
load("../Data/NOAA_stocks/Assessment_spatially_allocated.Rdata")
MEgrid <- subset(grid,grid$ME %in% c(ME_name1,ME_name2))
...
grid1 <- get_grid_ME(ME_name = ME_name1 , grid_size_km = 10)
grid2 <- get_grid_ME(ME_name = ME_name2 , grid_size_km = 10)
...
# select species with stock assessment
dem_stock <- c("Pleuronectes quadrituberculatus","Arctoraja parmifera",
               "Sebastes aleutianus","Sebastes melanostictus",
               "Atheresthes stomias","Pleurogrammus monopterygius",
               "Hippoglossoides elassodon","Reinhardtius hippoglossoides",
               "Atheresthes evermanni","Lepidopsetta polyxystra",
               "Sebastes polyspinis","Gadus macrocephalus",
               "Sebastes alutus","Sebastes borealis",
               "Gadus chalcogrammus","Limanda aspera",
               "Bathyraja aleutica","Bathyraja interrupta")

Reg$F_type <- ifelse(Reg$Name %in% dem_stock,"dem_stock", Reg$F_type)
...
grid <- subset(grid, grid$lon < -1) # to improve
...
Reg <- Reg %>%
  mutate(Month = as.numeric(Month), 
    ...
    Surveyf = as.factor(Survey_Region))  
...
if (!file.exists(paste0("../Models_sdmtmb/stocks_",ME_comb,".RData"))) {
  ...
  save(model_fit,file= paste0("../Models_sdmtmb/stocks_",ME_comb,".RData"))

} else {
  load(paste0("../Models_sdmtmb/stocks_",ME_comb,".RData"))
}
...
if (!file.exists(paste0("../Models_sdmtmb/Index_stocks_",ME_comb,".RData"))) {
  bio_index <- get_index(predictions, area = 100, bias_correct = T)
  save(bio_index,file= paste0("../Models_sdmtmb/Index_stocks_",ME_comb,".RData"))
} else {
  load(paste0("../Models_sdmtmb/Index_stocks_",ME_comb,".RData"))
}
stck_index <- bio_index
stck_index$area <- length(grid$Year[grid$Year == Start_year]) * 100

plot(tr$Bio/stck_index$area[1]~tr$Year,ylim=c(0,100),type="l",col="black",
...
if (!file.exists(paste0("../Models_sdmtmb/total_",ME_comb,".RData"))) {
  ...
  # Print model summary for diagnostics
  save(model_fit,file= paste0("../Models_sdmtmb/total_",ME_comb,".RData"))

} else {
  load(paste0("../Models_sdmtmb/total_",ME_comb,".RData"))
}
...
# Calculate the biomass index with bias correction
if (!file.exists(paste0("../Models_sdmtmb/Index_total_",ME_comb,".RData"))) {
  bio_index <- get_index(predictions, area = 100, bias_correct = T)
  save(bio_index,file= paste0("../Models_sdmtmb/Index_total_",ME_comb,".RData"))
} else {
  load(paste0("../Models_sdmtmb/Index_total_",ME_comb,".RData"))
}
...
lines(tr$ER~tr$Year,lty=1,lwd=2,col="black")
...
dat <- data.frame(Region = ME_name, 
           ...
           Stckbio_STO_MTkm2 = tr$Bio/stck_index$area[1],
