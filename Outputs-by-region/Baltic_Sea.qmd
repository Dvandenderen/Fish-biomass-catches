---
title: "Baltic Sea biomass and catches"
format: html
...
The catch and landings figures include a time series of landings for all assessed stocks for which total biomass or spawning stock biomass estimates are available. Some of these stocks have spatial assessment areas that only partially overlap with the area. In such cases, catches/landings were proportionally adjusted based on the degree of overlap. For each stock, catches/landings were allocated only to areas where the species occur. For example, catches/landings from stocks inhabiting shallow waters were not assigned to deeper areas. This allocation was based on distribution maps from AquaMaps.

```{r Catch & Land. }
# -----------------------------------------------------
# load SAU data
SAU <- read.csv("../Data/SAU/SAU_BalticSea.csv")
...
STO <- subset(STO,!(STO$Stock %in% c("dgs.27.nea","mac.27.nea","whb.27.1.91214")))
...
Time series of assessed fish biomass were estimated using data from scientific trawl surveys and based on information for all assessed stocks for which total biomass or spawning stock biomass estimates are available. Some of these stocks have spatial assessment areas that only partially overlap. In such cases, biomass were proportionally adjusted based on the degree of overlap. For each stock, biomass were allocated only to areas where the species occur. For example, biomass from stocks inhabiting shallow waters were not assigned to deeper areas. This allocation was based on distribution maps from AquaMaps.

The biomass estimates from scientific surveys were corrected for differences in sampling area and trawl gear catchability (Maureaud et al. 2024; van Denderen et al. 2023). We estimated both total biomass and biomass per unit area using the GLMMTMB package. We only included species for which a stock assessment is provided. For each of these species, we selected only the hauls that overlap with the stock assessment area for that species.
...
# for most stocks, uncertainty in Tbio is not provide; put median 
# do not plot or report upon
...
Reg <- subset(Reg,Reg$Survey_Region == "BITS") #rm few NS-IBTS hauls

# match all hauls with the LMEgrid id
hauls <- Reg[!duplicated(Reg$Haul_id), ]
hauls <- st_as_sf(hauls, coords = c("Longitude", "Latitude"), crs = 4326)
ha_grid <- st_intersects(hauls,LMEgrid)
ha_grid <- as.data.frame(ha_grid)
ha_grid <- data.frame(Haul_id = hauls$Haul_id[ha_grid[,1]],uni_id = LMEgrid$id[ha_grid[,2]])
...
spec <- stringr::str_sub(colnames(LMEgrid), 1, 3)
...
for (j in 1:length(spec)) {
  ...
  LMEgrid_df <- st_drop_geometry(LMEgrid)
  cols_with_spec <- grep(paste0("^", spec[j]), colnames(LMEgrid_df))
  if (length(cols_with_spec) > 1) {
    LMEgrid_df$new <- rowSums(LMEgrid_df[, cols_with_spec])
  } else {
    LMEgrid_df$new <- LMEgrid_df[, cols_with_spec]
  }
  
  areaspec <- subset(LMEgrid_df, LMEgrid_df$new == 1)
...
grid$col <- ifelse(grid$lat < 58,"blue","red")
...
if (!file.exists(paste0("../Models_sdmtmb/stocks_",LME_name,".RData"))) {
  ...
  # choice of family is tested through visual inspection of residuals 
  model_fit <- sdmTMB(
    stocks_kg_km2 ~ 0 + Yearf + Monthf +  
      poly(log_depth_std,2,raw = T), 
    data = Reg,                     
    mesh = spatial_mesh,                
    family = delta_lognormal(),            # many zero's 
...
grid <- subset(grid, grid$Latitude < 58)
...
if (!file.exists(paste0("../Models_sdmtmb_Pred/Prediction_",LME_name,".RData"))) {
...
# predictions$data$est <- exp(predictions$data$est)
  ...
  save(predictions,file= paste0("../Models_sdmtmb_Pred/Prediction_",LME_name,".RData"))

} else {
  load(paste0("../Models_sdmtmb_Pred/Prediction_",LME_name,".RData"))
}
...
Time series of the fish community exploitation rate were estimated by LME by dividing catches over total biomass. This was done using the SAU and Watson datasets divided by survey biomass, as well as, using stock assessment information on both biomass and catches.

Biomass estimates from the survey cover only about half of the Baltic Sea, the area included in scientific surveys, whereas catch data by LME encompass the entire region. To reconcile this difference, we used finer-scale data from Watson to estimate that approximately 65% of the total catch originates from the area overlapping with the survey predictions. We adjusted the catch data accordingly to better approximate an exploitation rate that corresponds to the biomass estimate from the surveyed area.

```{r Expl. rate}
# -----------------------------------------------------
plot((Catch_SAU$Tonnes*.65)/(bio_index$est/1000) ~ Catch_SAU$Year, 
     lty=2,lwd=2,col="red",type="l",ylab="yr-1",las=1,xlab="Year",
     main="Exploitation rate",ylim=c(0,1))

lines((Catch_WAT$Tonnes*.65)/(bio_index$est/1000) ~ Catch_WAT$Year,lty=3,lwd=2,col="blue")
...
In the catch statistics, demersal fish account for approximately 25% of the total catch, while in the survey data, they represent around 50%. This discrepancy likely reflects the underestimation of pelagic fish in survey data. Additionally, part of the difference may stem from the spatial mismatch: catch statistics cover the entire LME, whereas the surveys are limited to the southern part of the Baltic Sea.
...
write.csv(dat, paste0("../Outputs-by-region/",LME_name,".csv"),row.names = F)
